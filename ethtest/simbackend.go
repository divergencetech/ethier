...
// CollectCoverage starts an EVM trace on all future operations, collecting
// source-code coverage for those contracts mapped by the SourceMap.
//
// CollectCoverage sets the SimulatedBackend's Blockchain.VMConfig.Tracer which,
// if overwritten, will cause coverage collection to cease.
func (sb *SimulatedBackend) CollectCoverage(src *solidity.SourceMap) {
	cfg := sb.Blockchain().GetVMConfig()
	cfg.Debug = true

	coll, report := src.CoverageCollector()
	cfg.Tracer = coll
	sb.coverageReport = report
}

// CollectCoverageTB is a convenience method for CollectCoverage, accepting the
// SourceMap() function generated by `ethier gen`.
func (sb *SimulatedBackend) CollectCoverageTB(tb testing.TB, fn func() (*solidity.SourceMap, error)) {
	tb.Helper()
	src, err := fn()
	if err != nil {
		tb.Fatalf("%T error %v", fn, err)
	}
	sb.CollectCoverage(src)
}

// CoverageReport returns an LCOV trace file for contracts mapped by the
